...
export const WALL_OFFSET = 1.0;
export const WALL_H = 1.1;
export const EMB_BASE = 3;
export const EMB_SLOPE = 1.4;
...
export class Track {
    ...
    _build(pts, jumps, validate) {
        ...
        for (let i = 0; i < S; i++) {
            ...
            total += l; dx[i] = ddx / l; dz[i] = ddz / l; rx[i] = -dz[i]; rz[i] = dx[i];
        }
        ...
        const parseFeatures = (arr) => {
            ...
            for (const item of (arr || [])) {
                const p = vec[item.pt]; let best = 0, bd = Infinity;
                for (let i = 0; i < S; i++) { const a = px[i] - p.x, b = pz[i] - p.z, d = a * a + b * b; if (d < bd) { bd = d; best = i; } }
                const n = Math.max(3, Math.round(item.len / this.segLen));
                res.push({ ...item, i: best, n });
            }
            return res;
        };

        this.jumps = parseFeatures(jumps);
        for (const jp of this.jumps) {
            for (let m = 0; m <= jp.n; m++) { const i = ((jp.i - jp.n + m) % S + S) % S, f = m / jp.n; py[i] += jp.h * (0.5 * f + 0.5 * f * f); }
        }
        this.bridges = parseFeatures(this.def.bridges);
        this.tunnels = parseFeatures(this.def.tunnels);
        ...
        for (const jp of this.jumps) this.slope[jp.i] = this.slope[(jp.i - 1 + S) % S];
    ...
    build(quality = 1) {
        ...
        // 브릿지/터널 마스크 생성
        const bridgeMask = new Uint8Array(S), tunnelMask = new Uint8Array(S);
        for (const br of this.bridges) { const h = Math.floor(br.n / 2); for (let m = 0; m < br.n; m++) bridgeMask[(br.i - h + m + S) % S] = 1; }
        for (const tn of this.tunnels) { const h = Math.floor(tn.n / 2); for (let m = 0; m < tn.n; m++) tunnelMask[(tn.i - h + m + S) % S] = 1; }
        ...
        for (const s of [-1, 1]) {
            ...
            parts.push(this._strip(i => P(i, s * (WO + 0.3), 0), i => {
                if (bridgeMask[i]) {
                    const p = P(i, s * (WO + 0.3), 0);
                    p[1] = this.py[i] - 1.5; // 다리처럼 두께만 남기고 흙을 깎음 (길 안막힘!)
                    return p;
                }
                const p = P(i, s * (WO + 0.3 + this.py[i] * EMB_SLOPE + EMB_BASE), 0);
                p[1] = 0; return p;
            }, i => shade(emb, 0.9 + (i % 7) * 0.02)));
        }
        ...
        // 🚨 터널 3D 모델 렌더링 파트
        let tunnelCount = 0;
        for (let i = 0; i < S; i++) if (tunnelMask[i] && i % 2 === 0) tunnelCount++;
        if (tunnelCount > 0) {
            const archShape = new THREE.Shape();
            archShape.moveTo(-WO - 0.5, -0.5);
            archShape.lineTo(-WO - 0.5, WALL_H + 2);
            archShape.quadraticCurveTo(0, WALL_H + 5, WO + 0.5, WALL_H + 2);
            archShape.lineTo(WO + 0.5, -0.5);
            archShape.lineTo(WO - 0.5, -0.5);
            archShape.quadraticCurveTo(0, WALL_H + 4, -WO + 0.5, WALL_H + 2);
            archShape.lineTo(-WO + 0.5, -0.5);
            const archGeo = new THREE.ExtrudeGeometry(archShape, { depth: this.segLen * 2.1, bevelEnabled: false });
            archGeo.center();
            const tMat = new THREE.MeshLambertMaterial({ color: 0x151515, emissive: 0x110a00, side: THREE.DoubleSide });
            const im = new THREE.InstancedMesh(archGeo, tMat, tunnelCount);
            const obj = new THREE.Object3D();
            let k = 0;
            for (let i = 0; i < S; i++) {
                if (tunnelMask[i] && i % 2 === 0) {
                    obj.position.set(this.px[i], this.py[i] + 2.5, this.pz[i]);
                    obj.rotation.y = Math.atan2(-this.rz[i], this.rx[i]);
                    obj.updateMatrix();
                    im.setMatrixAt(k++, obj.matrix);
                }
            }
            g.add(im);
        }
