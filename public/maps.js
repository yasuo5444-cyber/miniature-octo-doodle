...
export const MAPS = {
    ...
    construction: {
        name: '사막 빙글빙글 공사장', icon: '🏗️', diff: 5, cp: 18, scale: 0.9,
        desc: '원작 완벽 구현! 거대한 나선형 미궁 탑과 대형 점프 (약 2,500m)',
        // 점차 바깥으로 넓어지는 나선형 구조로 겹침 버그를 완벽하게 차단하고 빙글빙글 코스를 구현!
        pts: [
            [0, -400, 0],         // 0. 출발선 (완벽한 직선)
            [250, -400, 0],       // 1. 긴 직선 후 첫 번째 우코너
            [250, -250, 0],       // 2. 직각 코너 (공사장 초입)
            [-50, -250, 0],       // 3. 직각 코너 (나선형 탑 진입로)
            [-50, -100, 4],       // 4. 나선형 탑 1층 진입
            [100, -100, 8],       // 5. 1층 우측
            [100, 100, 12],       // 6. 1층 상단
            [-150, 100, 16],      // 7. 2층 좌측 (여기서부터 회전반경이 점차 넓어집니다!)
            [-150, -150, 20],     // 8. 2층 하단
            [200, -150, 24],      // 9. 3층 우측
            [200, 200, 28],       // 10. 3층 상단
            [-250, 200, 32],      // 11. 4층 좌측
            [-250, -250, 36],     // 12. 4층 하단
            [350, -250, 42],      // 13. 5층 우측 (최정상 42m 도달!) -> 여기서 부스터 밟고 점프!
            [350, -400, 10]       // 14. 거대 점프 후 착지, 출발선으로 부드럽게 연결
        ],
        pads: [0.2, 0.4, 0.55, 0.75, 0.92], // 나선형 탑을 오르는 중간중간 부스터 패드 배치
        jumps: [{ pt: 13, len: 60, h: 5 }], // 13번 최정상 꼭대기에 출발선 쪽으로 날아가는 거대 점프대 설치!
        palette: {
            sky: 0xeb9f6c, fog: 0xdc8850, ground: 0xc28f5b, road: 0x3b3f45, // 사막 테마: 붉은 모래색과 노을진 하늘
            curbA: 0xffd23f, curbB: 0x111111, // 공사장 테마: 노랑/검정 경고 무늬 연석
            wall: 0x998877, emb: 0x886644, light: 0xffe6cc
        },
        decor: { crane: 25, container: 100, building: 15, rock: 60, cactus: 30 }, // 크레인과 컨테이너, 선인장 듬뿍 추가!
    }
}