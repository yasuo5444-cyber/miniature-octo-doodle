...
function createRoom() {
    const room = {
        code: newCode(), host: null, state: 'lobby', players: new Map(), race: null, tick: null,
    ...
    ws.on('message', (raw) => {
        ...
        switch (m.type) {
            ...
            case 'settings': {
                ...
                // [수정됨] 맵 데이터를 더 유연하게 받도록 처리
                const incomingMap = m.mapId || m.map;
                if (incomingMap) {
                    const safeMapId = String(incomingMap).toLowerCase().trim();
                    if (MAP_CP[safeMapId]) {
                        room.settings.mapId = safeMapId;
                        if (safeMapId === 'random') room.settings.seed = newSeed();
                    }
                }
